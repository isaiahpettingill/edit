...
You can find our translations in [`i18n/edit.toml`](./crates/edit/i18n/edit.toml).
...
* The underlying text buffer in `crates/edit/src/buffer` doesn't keep track of line breaks in the document.
  ...
  * As a result, `crates/edit/src/simd` contains crucial `memchr2` functions to quickly find the next or previous line break (runs at up to >100GB/s).
  * Furthermore, `crates/edit/src/unicode` implements an `Utf8Chars` iterator which transparently inserts U+FFFD replacements during iteration (runs at up to 4GB/s).
  * Furthermore, `crates/edit/src/unicode` also implements grapheme cluster segmentation and cluster width measurement via its `MeasurementConfig` (runs at up to 600MB/s).
...
* `crates/edit/src/framebuffer.rs` implements a "framebuffer" like in video games.
...
* `crates/edit/src/tui.rs` implements an immediate mode UI. Its module implementation gives an overview how it works and I recommend reading it.
* `crates/edit/src/vt.rs` implements our VT parser.
* `crates/edit/src/sys` contains our platform abstractions.
* Finally, `crates/edit/src/bin/chedit` ties everything together.
...
* VT parser in `crates/edit/src/vt.rs`
* Platform specific code in `crates/edit/src/sys`
* And the `setup_terminal` function in `crates/edit/src/bin/chedit/main.rs`
