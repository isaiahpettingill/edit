...
fn compile_lsh() {
    ...
    let lsh_path = lsh_lib::compiler::builtin_definitions_path();
    let out_dir = env_opt("OUT_DIR");
    let out_path = format!("{out_dir}/lsh_definitions.rs");

    let mut generator = lsh_lib::compiler::Generator::new(&scratch);
...
fn compile_i18n() {
    ...
    let i18n_path = std::path::Path::new(manifest_dir).join("i18n/edit.toml");
